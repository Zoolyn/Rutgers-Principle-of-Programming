...
(define updateAssocList
  (lambda (assocList symbol set)
    ...
    (cond
     ((null? assocList)
     '())
    ((eq? (car(car assocList)) symbol)(append (cons (cons symbol(cons (union (car(cdr (assoc (car(car assocList)) assocList))) set) '())) (updateAssocList (cdr assocList) symbol set))))
...
(define getInitSets
  (lambda (NTs startSymbol setting)
    ...
    (cond
      ...
      (else #f) ;if one of the settings was not picked correctly
      )
  ))
    ...
    (letrec ((first
              (lambda (symbolSeq firstSets)
                ...
                (if
                 ...
                 (cons (car symbolSeq) '()) ;;this is if is a terminal
                 )
              )))
      first)))
