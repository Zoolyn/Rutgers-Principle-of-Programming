...
static void assign()
{
	...
	//printf("this should be a identifier: %c\n", id);
	next_token();
...
static void print()
{
	...
	next_token();
	//need to make sure that this is a variable that follows a # symbol
	if(!is_identifier(token)){
		ERROR("Expected identifier");
		exit(EXIT_FAILURE);
	}
...
static void stmt()
{
	...
	//this is the case for read
	case '!':
		read();
		return;
	//this is for the case for print
	case '#':
		print();
		return;
	}
